      ...
      <div class="flex flex-wrap items-center">
        <div class="relative w-full px-4 max-w-full flex-grow flex-1">
          <h3 class="font-semibold text-lg" :class="[color === 'light' ? 'text-blueGray-700' : 'text-white']">
            Người dùng
            <!-- <router-link to="/admin/users/create" class="absolute right-0">
              <button
          ...
          <tr v-for="user in users" :key="user._id">
            ...
            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
              <router-link :to="`/admin/users/${user._id}`">
                <button class="text-lg fa-solid fa-pen-to-square mr-5"></button>
              </router-link>
              <button class="text-lg text-red-600 fa-regular fa-trash-can" @click="deleteUser(user._id)"></button>
            </td> -->
            <!-- <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
              <div class="flex">
...
export default {
  ...
  methods: {
    async getUsers(q) {
      this.users = (await this.userService.getAll({ q, perPage: 999 }))?.data?.docs
    },
    async refresh() {
      this.getUsers()
    },
