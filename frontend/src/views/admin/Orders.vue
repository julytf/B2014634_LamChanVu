      ...
      <div class="flex flex-wrap items-center">
        <div class="relative w-full px-4 max-w-full flex-grow flex-1">
          <h3 class="font-semibold text-lg" :class="[color === 'light' ? 'text-blueGray-700' : 'text-white']">
            Đơn hàng
            <!-- <router-link to="/admin/orders/create" class="absolute right-0">
              <button
      ...
      <table class="items-center w-full bg-transparent border-collapse">
        ...
        <tbody>
          <tr v-for="order in orders" :key="order._id">
            ...
            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
              {{ order.details.reduce((p, c) => p + (c.quantity * c.book.price), 0) }} vnđ
            </td>
            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
              {{ order.status }}
            </td>
            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
              <div v-if="order.status != 'Đã hủy'">
                <button v-if="order.status == 'Chờ duyệt'"
                  class="bg-emerald-500 text-white active:bg-emerald-600 text-xs font-bold uppercase px-4 py-2 rounded shadow hover:shadow-lg outline-none focus:outline-none lg:mr-1 lg:mb-0 ml-3 mb-3 ease-linear transition-all duration-150"
                  type="button" @click="acceptOrder(order._id, order)">
                  Duyệt
                </button>
                <button v-if="order.status == 'Đã duyệt'"
                  ...
                  type="button" @click="shippedOrder(order._id, order)">
                  Giao hàng
                </button>
                <button
                  ...
                  type="button" @click="cancelOrder(order._id, order)">
                  Hủy
                </button>
              </div>
            </td>
