  ...
  <nav
    ...
    <div class="container px-4 mx-auto flex flex-wrap items-center justify-between">
      <div class="w-full relative flex justify-between lg:w-auto lg:static lg:block lg:justify-start">
        <router-link to="/">
          <a class="text-blueGray-700 text-2xl font-bold leading-relaxed inline-block mr-4 py-2 whitespace-nowrap uppercase"
            href="#pablo">
            FHS
          </a>
        </router-link>
      ...
      <div class="lg:flex flex-grow items-center" :class="[navbarOpen ? 'block' : 'hidden']" id="example-navbar-warning">
        ...
        <ul v-if="authStore.token == null" class="flex flex-col lg:flex-row list-none lg:ml-auto">
          <!-- <li class="flex items-center">
            <index-dropdown />
          </li>
          <li class="flex items-center">
          ...
          <li class="flex items-center">
            <router-link to="/auth/register">
              <button
                class=" text-emerald-600 active:bg-emerald-600 active:text-white text-xs font-bold uppercase px-4 py-2 rounded shadow hover:shadow-lg outline-none focus:outline-none lg:mr-1 lg:mb-0 ml-3 mb-3 ease-linear transition-all duration-150"
                type="button">
                Đăng ký
              </button>
            </router-link>
        ...
        <ul v-if="authStore.token != null" class="flex flex-col lg:flex-row list-none lg:ml-auto">
          ...
          <li class="flex items-center">
            <router-link to="/orders">
              <button
                class=" text-emerald-600 active:bg-emerald-600 active:text-white ring-2 ring-emerald-500 text-xs font-bold uppercase px-4 py-2 rounded shadow hover:shadow-lg outline-none focus:outline-none lg:mr-1 lg:mb-0 ml-3 mb-3 ease-linear transition-all duration-150"
                type="button">
                Đơn hàng
              </button>
            </router-link>
          ...
          <li class="flex items-center">
            <router-link to="/cart">
              <button
                class=" text-emerald-600 active:bg-emerald-600 active:text-white ring-2 ring-emerald-500 text-xs font-bold uppercase px-4 py-2 rounded shadow hover:shadow-lg outline-none focus:outline-none lg:mr-1 lg:mb-0 ml-3 mb-3 ease-linear transition-all duration-150"
                type="button">
                Giỏ hàng ({{ cartStore.details?.reduce((p, c) => { return p + c.quantity }, 0) ?? 0 }})
              </button>
            </router-link>
          </li>
          <li class="flex items-center">
            <div>
              <button
                class=" text-emerald-600  text-xs font-bold uppercase px-4 py-2 rounded shadow hover:shadow-lg outline-none focus:outline-none lg:mr-1 lg:mb-0 ml-3 mb-3 ease-linear transition-all duration-150"
                type="button">
